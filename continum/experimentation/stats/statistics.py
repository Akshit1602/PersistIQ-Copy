...
def proportion_test(
    ...
    se     = np.sqrt(p_pool * (1 - p_pool) * (1 / n_ctrl + 1 / n_treat)) if (n_ctrl + n_treat) > 0 else 1.0
    z_stat = delta / se if se > 0 else 0.0
    p_val  = 2 * norm.sf(abs(z_stat))

    se_delta = np.sqrt(p_c * (1 - p_c) / n_ctrl + p_t * (1 - p_t) / n_treat) if (n_ctrl * n_treat) > 0 else 0.0
...
def means_test(
    ...
    if np.isnan(p_val): p_val = 1.0  # identical arrays or zero variance
...
def check_srm(
    ...
    if expected_fractions is None:
        ...
        expected = np.array([expected_fractions.get(v, 1 / len(variants)) * n_total for v in variants])
...
__all__ = [
    "winsorise", "safe_val",
    "proportion_test", "means_test",
    "compute_sample_size", "compute_duration",
    "check_srm", "compute_opportunity",
