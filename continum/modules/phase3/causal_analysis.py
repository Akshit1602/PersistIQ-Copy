...
logger = logging.getLogger("continum.phase3.causal")
...
def run_causal_analysis_full(
    ...
    **kwargs,
    ...
    if not method:
        ...
        method_map = {
            "1": "did", "2": "its", "3": "psm", "4": "sc",
            "5": "mediation", "6": "rdd", "7": "simpsons", "8": "all",
        }
    ...
    if method in ("did", "all"):
        try:
            from continum.core.causal.engine import run_did_v2
            segments   = df["account_segment"].dropna().unique().tolist() if "account_segment" in df.columns else []
            ...
            did_result = run_did_v2(
                df=df, treatment_units=trt_units, control_units=ctrl_units,
                cutoff_date=cutoff, pre_start=pre,
    ...
    if method in ("its", "all"):
        try:
            from continum.core.causal.engine import run_its_enhanced
    ...
    if method in ("psm", "all"):
        try:
            from continum.core.causal.engine import run_psm_full
            from continum.core.causal.balance import plot_love_plot, print_balance_report
            psm_result = run_psm_full(
                df=df, outcome_col="converted_to_order",
                treatment_col="variant", control_label="control",
            )
            ...
            if "smd_after" in psm_result:
                ...
                bal_full = {k: {"covariate": k, "smd": v["smd"],
                                "flag": v["smd"] > 0.10, "mean_ctrl": 0, "mean_trt": 0}
                            for k, v in bal.items()}
                plot_love_plot(bal_full, experiment_name)
                print(f"  📁 Love plot saved → love_plot.png")
        except Exception as e:
            results["psm"] = {"error": str(e)}

    if method in ("sc", "all"):
        try:
            from continum.core.causal.engine import run_synthetic_control_v2
            if "account_segment" in df.columns:
                ...
                if len(segs) >= 3:
                    ...
                    sc_result = run_synthetic_control_v2(
                        df=df, treatment_unit=trt_seg, donor_units=donors,
                        cutoff_date=cutoff, pre_start=pre,
                    )
    ...
    if method in ("mediation", "all"):
        try:
            from continum.core.causal.engine import run_mediation_analysis
            med_col = next((c for c in ["has_billing_profile", "is_active_30d", "platform"]
                            if c in df.columns), None)
            if med_col:
                med_result = run_mediation_analysis(
                    df=df, treatment_col="variant",
                    mediator_col=med_col, outcome_col="converted_to_order",
                )
    ...
    if method in ("rdd", "all"):
        try:
            from continum.core.causal.engine import run_rdd_enhanced
            run_col = next((c for c in ["propensity_score", "lifetime_orders", "personal_ior"]
                            if c in df.columns), None)
            if run_col:
                cutoff_val = float(df[run_col].dropna().median())
                rdd_result = run_rdd_enhanced(
                    df=df, running_var=run_col, cutoff=cutoff_val,
                )
    ...
    if method in ("simpsons", "all"):
        try:
            from continum.core.causal.engine import detect_simpsons_paradox
    ...
    try:
        from continum.core.intelligence.analytical_reasoning import mine_additional_insights
        insights = mine_additional_insights(
            df=df, control=control, treatment=treatment,
        )
    ...
    if llm is not None:
        try:
            from continum.core.intelligence.analytical_reasoning import synthesise_findings
            ctx = {
                "experiment": experiment_name,
                ...
                "reasoning": "",
                "overall_summary": str({k: v.get("did_estimate_pp") or v.get("level_change_pp")
                                        or v.get("att_pp") for k, v in results.items()
                                        if isinstance(v, dict) and not v.get("error")})[:300],
                "segment_summary": "",
                "interesting_summary": "",
                "time_trend_summary": results.get("additional_insights", {}).get("time_decay", {}).get("summary", ""),
                "extra_insights": "",
                "past_learnings": "",
                "n_past_learnings": 0,
            }
    ...
    try:
        ...
        for m_key, m_result in results.items():
            if isinstance(m_result, dict) and not m_result.get("error"):
                est = (m_result.get("did_estimate_pp") or m_result.get("level_change_pp")
                       or m_result.get("att_pp") or m_result.get("avg_lift_pp") or 0)
                bus.emit(
                    source="causal_analysis",
                    insight_type=__import__("continum.runtime.intelligence",
                                           fromlist=["InsightType"]).InsightType.CAUSAL,
                    severity=__import__("continum.runtime.intelligence",
                                        fromlist=["InsightSeverity"]).InsightSeverity.INFO,
...
def _print_causal_result(method_name: str, result: Dict) -> None:
    ...
    print(f"\n  ── {method_name} ─────────────────────────────────────────────")
    keys_of_interest = [
        "did_estimate_pp", "level_change_pp", "att_pp", "avg_lift_pp",
        "discontinuity_pp", "indirect_effect_pp", "p_value",
        "is_significant", "parallel_trends_ok", "balance_ok",
        "fit_note", "balance_note", "parallel_trends_note", "verdict",
        "manipulation_note",
    ]
