...
def _ck(kw):
    return {k: v for k, v in kw.items() if k not in _STRIP_KEYS}
...
# ─────────────────────────────────────────────────────────────────────────────
# DISPLAY NAMES — the single source of truth for module identity.
#
# Every UI surface (dashboard cards, modal headers, run/result labels, chat
# replies, guided-flow prompts, MatchView tool confirmations) must read a
# module's name from here — never hardcode a display string elsewhere. See
# .claude/skills/module-registry for the full rule set.
# ─────────────────────────────────────────────────────────────────────────────


def default_display_name(name: str) -> str:
    """Title-case a module key, one word at a time (mirrors the dashboard's JS
    ``humanize()`` exactly, so an unregistered override never drifts between
    the Python and JS defaults)."""
    return " ".join(w[:1].upper() + w[1:] for w in name.split("_") if w)


# Curated names where the default title-casing reads wrong (acronyms,
# apostrophes) or where the module is deliberately renamed for the user
# (e.g. "experiment_analysis" is shown as "A/B Readout" everywhere).
_DISPLAY_NAME_OVERRIDES: Dict[str, str] = {
    ...
    "roi_synthesis": "ROI Synthesis",
    "kpi_synthesis": "KPI Synthesis",
}
...
class ModuleSpec:
    def __init__(
        ...
        self.display_name = (
            display_name or _DISPLAY_NAME_OVERRIDES.get(name) or default_display_name(name)
        )
...
def list_modules() -> List[Dict]:
    ...
    specs = [s for k, s in _REGISTRY.items() if not k.startswith("_") and isinstance(s, ModuleSpec)]
    return [
        {
            "name": s.name,
...
def run_module(name: str = None, state=None, llm=None, db=None, **kwargs) -> Any:
    ...
    try:
        ...
        try:
            from continum.ContextGraph.insight_bus import get_bus, publish_next_steps
...
# REGISTRY BUILDER
#
# Builds the full module registry in one idempotent pass. This was historically
# four separate `_build_registry` definitions monkey-patched on top of each other
# (base foundation → intelligence/forecasting/roi → V2 notebook modules → V3
# autonomous re-registrations); they are now a single function. ORDERING MATTERS:
# the later "autonomous re-registration" blocks intentionally pop-and-re-register
# earlier module names with fuller implementations, so they must run last.
# ─────────────────────────────────────────────────────────────────────────────
...
def _build_registry() -> None:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # BASE — Phase 0..4 foundation modules
    # ══════════════════════════════════════════════════════════════════════════

    # ── Phase 0 — Foundation ─────────────────────────────────────────────────

    try:
        from continum.mapMeta.discovery import (
            run_data_validation,
            run_dimension_setup,
        ...
        register_module(
            ModuleSpec(
                "schema_discovery",
                lambda state, llm=None, db=None, **kw: run_schema_discovery(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_0",
                "discovery",
        ...
        register_module(
            ModuleSpec(
                "data_validation",
                lambda state, llm=None, db=None, **kw: run_data_validation(db=db, llm=llm),
                "phase_0",
                "validation",
        ...
        register_module(
            ModuleSpec(
                "dimension_setup",
                lambda state, llm=None, db=None, **kw: run_dimension_setup(db=db, llm=llm),
                "phase_0",
                "setup",
    ...
    try:
        from continum.ExpSuite.discovery.monitors import (
        ...
        register_module(
            ModuleSpec(
                "pipeline_health",
                lambda state, llm=None, db=None, **kw: PipelineHealthMonitor(db=db).run(),
                "phase_0",
                "monitoring",
        ...
        register_module(
            ModuleSpec(
                "watchtower",
                lambda state, llm=None, db=None, **kw: WatchtowerMonitor(db=db).run(),
                "phase_0",
                "monitoring",
    ...
    try:
        from continum.ExpSuite.planning.planning import (
        ...
        register_module(
            ModuleSpec(
                "power_calculator",
                lambda state, llm=None, db=None, **kw: run_power_calculator(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_1",
                "planning",
        ...
        register_module(
            ModuleSpec(
                "opportunity_sizing",
                lambda state, llm=None, db=None, **kw: run_opportunity_sizing(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_1",
                "planning",
        ...
        register_module(
            ModuleSpec(
                "audience_selection",
                lambda state, llm=None, db=None, **kw: run_audience_selection(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_1",
                "planning",
        ...
        register_module(
            ModuleSpec(
                "brief_generator",
                lambda state, llm=None, db=None, **kw: run_brief_generator(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_1",
                "planning",
        ...
        register_module(
            ModuleSpec(
                ...
                lambda state, llm=None, db=None, **kw: run_metrics_and_tracking(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_1",
                "planning",
    ...
    try:
        from continum.ExpSuite.monitoring.monitoring import (
            run_health_monitor,
        ...
        register_module(
            ModuleSpec(
                "health_monitor",
                lambda state, llm=None, db=None, **kw: run_health_monitor(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_2",
                "monitoring",
        ...
        register_module(
            ModuleSpec(
                "sequential_testing",
                lambda state, llm=None, db=None, **kw: run_sequential_testing(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_2",
                "sequential",
    ...
    try:
        ...
        def _run_post_exp(
            state, llm=None, db=None, experiment_id: str = "", experiment_name: str = "", **kw
        ):
            return run_experiment_analysis_pipeline(
                ...
                llm=llm,
                knowledge_graph=(
                    getattr(getattr(state, "org", None), "_kg", None) if state else None
                ),
                **_ck(kw),
            )
    ...
    try:
        from continum.ExpSuite.analysis.post_analysis import (
            run_causal_analysis,
        ...
        register_module(
            ModuleSpec(
                "causal_analysis",
                lambda state, llm=None, db=None, **kw: run_causal_analysis(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_3",
                "causal",
        ...
        register_module(
            ModuleSpec(
                "pre_post_analysis",
                lambda state, llm=None, db=None, **kw: run_pre_post_analysis(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_3",
                "analysis",
        ...
        register_module(
            ModuleSpec(
                "simpsons_paradox",
                lambda state, llm=None, db=None, **kw: run_simpsons_paradox_detector(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_3",
                "analysis",
        ...
        register_module(
            ModuleSpec(
                "roi_tracker",
                lambda state, llm=None, db=None, **kw: run_roi_tracker(llm=llm, db=db, **_ck(kw)),
                "phase_3",
                "roi",
        ...
        register_module(
            ModuleSpec(
                ...
                lambda state, llm=None, db=None, **kw: run_learnings_repository(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_3",
                "knowledge",
    ...
    try:
        from continum.ExpSuite.discovery.monitors import SequentialTester
    ...
    try:
        from continum.ExpSuite.analysis.deployment import run_decision_engine, run_uplift_modeller

        register_module(
            ModuleSpec(
                "uplift_modeller",
                lambda state, llm=None, db=None, **kw: run_uplift_modeller(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_4",
                "deploy",
        ...
        register_module(
            ModuleSpec(
                "decision_engine",
                lambda state, llm=None, db=None, **kw: run_decision_engine(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_4",
                "deploy",
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Intelligence synthesis + Forecasting + ROI synthesis
    # ══════════════════════════════════════════════════════════════════════════

    # ── Intelligence synthesis modules ─────────────────────────────────────────
    try:
        from continum.ExpSuite.analysis.synthesis import (
        ...
        register_module(
            ModuleSpec(
                ...
                lambda state, llm=None, db=None, **kw: run_guardrail_generation(
                    llm=llm, db=db, **_ck(kw)
                ),
                "intelligence",
                "guardrails",
        ...
        register_module(
            ModuleSpec(
                "historical_learning",
                lambda state, llm=None, db=None, **kw: run_historical_learning_retrieval(
                    llm=llm, db=db, **_ck(kw)
                ),
                "intelligence",
                "knowledge",
        ...
        register_module(
            ModuleSpec(
                ...
                lambda state, llm=None, db=None, **kw: run_next_step_generation(
                    llm=llm, db=db, **_ck(kw)
                ),
                "intelligence",
                "planning",
        ...
        register_module(
            ModuleSpec(
                "anomaly_synthesis",
                lambda state, llm=None, db=None, **kw: run_anomaly_synthesis(
                    llm=llm, db=db, **_ck(kw)
                ),
                "intelligence",
                "monitoring",
        ...
        register_module(
            ModuleSpec(
                ...
                lambda state, llm=None, db=None, **kw: run_cross_experiment_learning(
                    llm=llm, db=db, **_ck(kw)
                ),
                "intelligence",
                "knowledge",
    ...
    # ── Forecasting modules ───────────────────────────────────────────────────
    try:
        from continum.ExpSuite.analysis.causal.forecasting import (
            run_arima_forecast,  # noqa: F401
            run_bsts_forecast,  # noqa: F401
            run_causal_impact,  # noqa: F401
            run_ets_forecast,  # noqa: F401
            run_forecast_ensemble,
            run_sarima_forecast,  # noqa: F401
        )
        ...
        def _run_forecasting(
            ...
            **kw,
            ...
            try:
                ...
                series = pd.Series(df["ior"].values, index=pd.to_datetime(df["date"]))
    ...
    # ── ROI Synthesis ─────────────────────────────────────────────────────────
    try:
        from continum.ExpSuite.analysis.roi_synthesis import run_roi_synthesis

        def _run_roi_synthesis_module(
            state, llm=None, db=None, experiment_name: str = "", ship_date: str = "", **kw
        ):
            if db is None:
                return {"error": "no_db"}
            try:
                ...
                return run_roi_synthesis(
                    df=df, ship_date=ship_date, experiment_name=experiment_name, **kw
                )
            except Exception as e:
                return {"error": str(e)}
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Ported notebook analytical modules
    # ══════════════════════════════════════════════════════════════════════════

    # ── Full causal analysis (engine.py — all 7 methods) ────────────────────
    try:
        from continum.ExpSuite.analysis.post_causal import run_causal_analysis_full

        register_module(
            ModuleSpec(
                "causal_analysis_full",
                lambda state, llm=None, db=None, **kw: run_causal_analysis_full(
                    llm=llm, db=db, **_ck(kw)
                ),
                "phase_3",
                "causal",
    ...
    try:
        from continum.ExpSuite.analysis.causal.engine import detect_distribution_shift
    ...
    try:
        from continum.ExpSuite.discovery.opportunity import (
            FUNNEL_TAXONOMY,
            classify_feature,
        ...
        def _run_opp(state, llm=None, db=None, description="", category="conversion", **kw):
    ...
    try:
        from continum.AskData.InsightGenerator import run_ask
    ...
    try:
        from continum.ExpSuite.analysis.analytical_reasoning import adaptive_recommendations
    ...
    try:
        from continum.ExpSuite.analysis.analytical_reasoning import generate_open_questions

        def _run_open_q(state, llm=None, db=None, experiment_name="", **kw):
    ...
    try:
        from continum.ExpSuite.analysis.analytical_reasoning import generate_root_cause

        def _run_root_cause(state, llm=None, db=None, experiment_name="", **kw):
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Autonomous re-registrations — swap thin module wrappers above for the full
    # autonomous implementations. These pop-and-re-register earlier names, so they
    # MUST run after all base/notebook registrations.
    # ══════════════════════════════════════════════════════════════════════════
            ...
            register_module(
                ModuleSpec(
                    ...
                    lambda state, llm=None, db=None, _fn=fn, **kw: _fn(
                        llm=llm, db=db, **{k: v for k, v in kw.items() if k not in _STRIP_KEYS}
                    ),
                    phase,
                    cat,
                    desc,
    ...
    # ── Phase 3 — no autonomous re-registration ──────────────────────────────
    # The base Phase-3 registrations above are authoritative. (A former V3 block
    # tried to re-register them but imported a `run_experiment_analysis` symbol
    # that does not exist in post_analysis.analysis — the real readout entry is
    # run_experiment_analysis_pipeline in analysis_dag, already wired as the
    # `experiment_analysis` module — so the block raised ImportError and was dead.
    # It only ever differed by description text, so it has been removed.)

    # ── Re-register Phase 4 ────────────────────────────────────────────────────
    try:
        from continum.ExpSuite.analysis.deployment import (
            run_decision_engine,
            run_uplift_modeller,
        )
    ...
    try:
        from continum.ExpSuite.modules import (
            run_audience_selection,
    ...
    logger.info("Module registry built: %d modules", n)
