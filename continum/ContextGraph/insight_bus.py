...
logger = logging.getLogger("continum.ContextGraph.insight_bus")
...
class InsightBus:
    ...
    def warn(self, source: str, message: str, detail: str = "", **kw) -> Insight:
        return self.emit(
            source, message, InsightType.WARNING, InsightSeverity.WARNING, detail=detail, **kw
        )

    def critical(self, source: str, message: str, detail: str = "", **kw) -> Insight:
        return self.emit(
            source, message, InsightType.WARNING, InsightSeverity.CRITICAL, detail=detail, **kw
        )

    def recommend(self, source: str, message: str, detail: str = "", **kw) -> Insight:
        return self.emit(
            source, message, InsightType.RECOMMENDATION, InsightSeverity.INFO, detail=detail, **kw
        )

    def suggest_kpi(self, source: str, message: str, metric: str = "", **kw) -> Insight:
        return self.emit(
            source, message, InsightType.KPI_SUGGESTION, InsightSeverity.INFO, metric=metric, **kw
        )

    def anomaly(self, source: str, message: str, detail: str = "", **kw) -> Insight:
        return self.emit(
            source, message, InsightType.ANOMALY, InsightSeverity.WARNING, detail=detail, **kw
        )
...
WORKFLOW_CHAIN: dict = {
    "schema_discovery": [
        ("power_calculator", "Run power analysis on discovered schema", 2),
        ("data_validation", "Validate data quality before experimenting", 1),
    ],
    "data_validation": [
        ("dimension_setup", "Auto-detect segments and dimensions", 1),
        ("opportunity_sizing", "Quantify the revenue opportunity", 2),
    ],
    "dimension_setup": [("opportunity_sizing", "Size the revenue opportunity", 1)],
    "opportunity_sizing": [
        ("power_calculator", "Calculate sample size for this opportunity", 1),
        ("brief_generator", "Generate experiment brief", 2),
    ],
    "power_calculator": [
        ("brief_generator", "Generate full experiment brief", 1),
        ("audience_selection", "Select and score target audience", 2),
    ],
    "brief_generator": [
        ("audience_selection", "Define audience for this experiment", 1),
        ("metrics_and_tracking", "Build KPI & tracking plan", 2),
    ],
    "audience_selection": [
        ("health_monitor", "Monitor experiment health once launched", 2),
    ...
    "health_monitor": [
        ("experiment_analysis", "Run full post-experiment readout", 1),
...
def publish_next_steps(module: str, bus: Optional[InsightBus] = None) -> None:
    ...
    try:
        from continum.ExpSuite.analysis.synthesis import (
            _PHASE_TRANSITIONS,
            run_next_step_generation,  # noqa: F401
        )
