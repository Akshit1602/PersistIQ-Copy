...
def generate_all(
    output_dir: str = "./sample_data/Xometry", n_users: int = 25_000, seed: int = 42
) -> dict:
...
def _gen_accounts(rng, n_users: int) -> pd.DataFrame:
    ...
    seg_vals = rng.choice(SEGMENTS, size=n_accounts, p=[SEGMENT_WEIGHTS[s] for s in SEGMENTS])
    return pd.DataFrame(
        {
            ...
            "country": rng.choice(COUNTRIES, size=n_accounts, p=[0.55, 0.15, 0.12, 0.10, 0.08]),
        }
    )
    ...
    return pd.DataFrame(
        {
            ...
            "_updated": created_at + pd.to_timedelta(rng.integers(0, 90, size=n_users), unit="D"),
            "_deleted": False,
            ...
            "email_flag": rng.choice(
                ["COMPANY_EMAIL", "PERSONAL_EMAIL"], size=n_users, p=[0.60, 0.40]
            ),
            ...
            "is_instant_quote_customer_user": rng.choice(
                [True, False], size=n_users, p=[0.30, 0.70]
            ),
...
def _gen_quotes_orders(rng, users_df: pd.DataFrame, accounts_df: pd.DataFrame):
    acc_map = accounts_df.set_index("_id")
    rows_q, rows_o = [], []
    q_idx = 0
    o_idx = 0
            ...
            rows_q.append(
                {
                    ...
                    "order_time": (
                        created + timedelta(days=int(rng.integers(1, 10))) if converted else None
                    ),
                    "order_time_et": None,
                    "user": user_id,
                    ...
                    "quote_source": rng.choice(
                        ["instant_quote", "manual", "rfq"], p=[0.6, 0.25, 0.15]
                    ),
                    ...
                    "selected_price_tier": rng.choice(
                        ["standard", "premium", "economy"], p=[0.6, 0.2, 0.2]
                    ),
                    ...
                    "shipping_method": rng.choice(
                        ["standard", "expedited", "overnight"], p=[0.7, 0.2, 0.1]
                    ),
                    ...
                    "calendar_days_to_order_conversion": (
                        int(rng.integers(1, 14)) if converted else None
                    ),
                    "business_days_to_order_conversion": (
                        int(rng.integers(1, 10)) if converted else None
                    ),
                }
            )
            q_idx += 1
                ...
                rows_o.append(
                    {
                        ...
                        "order_ship_status": rng.choice(
                            ["On Time", "Late", "Early"], p=[0.75, 0.15, 0.10]
                        ),
                        "asana_late_shipment_category": None,
                        "has_new_processes": False,
                        "has_rma_job": False,
                        "payment_type": rng.choice(
                            ["credit_card", "net30", "po"], p=[0.60, 0.30, 0.10]
                        ),
...
def _gen_experiments(rng, users_df: pd.DataFrame, accounts_df: pd.DataFrame) -> pd.DataFrame:
    acc_map = accounts_df.set_index("_id")
    rows = []
    ...
    for exp in EXPERIMENT_REGISTRY:
        ...
        for idx in sampled_idx:
            ...
            ts = start + pd.to_timedelta(int(rng.integers(0, max(1, (end - start).days))), unit="D")
            ...
            rows.append(
                {
                    ...
                    "order_id": (
                        f"ORD{rng.integers(0, 99999999):08d}" if rng.random() < 0.18 else None
                    ),
                    "job_id": None,
                    "partner_id": None,
...
def ensure_sample_data(output_dir: str = "./sample_data/Xometry") -> bool:
