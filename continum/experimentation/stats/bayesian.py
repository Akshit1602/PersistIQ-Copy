...
def beta_binomial_test(
    ...
    return {
        ...
        "posterior_alpha_ctrl": round(alpha_c, 2),
        "posterior_beta_ctrl": round(beta_c, 2),
        "posterior_alpha_treat": round(alpha_t, 2),
        "posterior_beta_treat": round(beta_t, 2),
        # Posterior summaries
...
def bayesian_multi_arm(
    ...
    expected_loss = {
        arm: float(np.mean(max_per_sample - samples[:, i])) for i, arm in enumerate(arms)
    }
...
def _beta_gt_prob(alpha1: float, beta1: float, alpha2: float, beta2: float) -> float:
...
def _hdi(samples: np.ndarray, credible_mass: float = 0.95) -> Tuple[float, float]:
    ...
    return (float(sorted_samples[min_idx]), float(sorted_samples[min_idx + interval_idx]))


def _savage_dickey_bf(alpha_t: float, beta_t: float, alpha_c: float, beta_c: float) -> float:
    try:
