...
FUNNEL_TAXONOMY: Dict[str, Dict] = {
    "acquisition": {
        ...
        "keywords":        ["login","sign","auth","oauth","register","onboard",
                            "traffic","landing","seo","ad","referral","invite"],
        "mde_benchmarks":  {"min_rel": 0.10, "max_rel": 0.30,
                            "label": "10–30% relative lift on sign-up rate"},
        "tracking_events": ["page_view","signup_start","signup_complete",
                            "verification_email_sent","signup_abandoned"],
        "guardrail_pool":  ["email_verification_rate","page_load_time_p95",
                            "backend_error_rate","signup_error_rate"],
        "questions": [
    ...
    "activation": {
        ...
        "keywords":        ["onboard","first","welcome","setup","profile","wizard",
                            "activation","tutorial","nudge","email"],
        "mde_benchmarks":  {"min_rel": 0.05, "max_rel": 0.20,
                            "label": "5–20% relative lift on activation rate"},
        "tracking_events": ["onboarding_step_completed","onboarding_completed",
                            "first_action","profile_completed","nudge_sent"],
        "guardrail_pool":  ["onboarding_abandonment_rate","time_to_activation",
    ...
    "conversion": {
        ...
        "keywords":        ["checkout","funnel","cart","billing","payment","order","quote",
                            "ior","conversion","buy","purchase","accept","submit","lead","sale"],
        "mde_benchmarks":  {"min_rel": 0.05, "max_rel": 0.15,
                            "label": "5–15% relative IOR lift (0.5–2pp absolute)"},
        "tracking_events": ["checkout_initiated","checkout_step_completed","checkout_completed",
                            "checkout_abandoned","payment_submitted","payment_failed",
                            "order_confirmed","quote_viewed","quote_accepted"],
        "guardrail_pool":  ["payment_failure_rate","checkout_error_rate",
                            "page_load_p95","support_ticket_rate"],
        "questions": [
    ...
    "retention": {
        ...
        "keywords":        ["repeat","retention","return","reorder","post.order",
                            "ltv","churn","re-engage","loyalty","upsell"],
        "mde_benchmarks":  {"min_rel": 0.05, "max_rel": 0.15,
                            "label": "5–15% relative lift on repeat order rate"},
        "tracking_events": ["reorder_initiated","repeat_order_confirmed",
                            "reengagement_email_opened","loyalty_action"],
    ...
    "engagement": {
        ...
        "keywords":        ["ui","ux","design","layout","search","recommend","notification",
                            "banner","modal","tooltip","button","cta","page"],
        "mde_benchmarks":  {"min_rel": 0.03, "max_rel": 0.10,
                            "label": "3–10% relative lift (UX changes tend to be smaller)"},
        "tracking_events": ["feature_clicked","feature_viewed","cta_clicked",
                            "notification_opened","scroll_depth_reached"],
    ...
    "pricing": {
        ...
        "primary_metric":  "ior",
        "keywords":        ["price","pricing","discount","fee","rate","tier","plan",
                            "monetis","anchor","promo","coupon","revenue"],
        "mde_benchmarks":  {"min_rel": 0.03, "max_rel": 0.12,
                            "label": "3–12% relative lift (pricing changes have mixed effects)"},
        "tracking_events": ["pricing_page_viewed","plan_selected","discount_applied",
                            "price_compared","upsell_accepted"],
        "guardrail_pool":  ["gross_margin","refund_rate","churn_rate"],
        "questions": [
            ("monthly_inquiries", "Monthly quotes that see the pricing change", "inquiries/mo", False),
...
def compute_dynamic_opportunity(
    ...
    if category == "acquisition":
        ...
        funnel_chain = [
            ...
            ("IOR",                f"{curr_ior*100:.2f}%",    f"{target_ior*100:.2f}%",  f"+{(target_ior-curr_ior)*100:.3f}pp"),
            ("Extra orders/mo",    f"{extra_orders:,.1f}",    "",                        ""),
        ]
        if category == "pricing" and abs(aov_mult - 1) > 0.001:
            aov_gmv = monthly_inq * curr_ior * (eff_aov - aov)
            funnel_chain.append(("AOV uplift", f"${aov:,.0f}", f"${eff_aov:,.0f}",
                                  f"+${aov_gmv:,.0f} GMV/mo"))
            aov = eff_aov
        ...
        funnel_chain = [
            ...
            ("Repeat rate",       f"{curr_rep*100:.2f}%",   f"{target_rep*100:.2f}%",  f"+{(target_rep-curr_rep)*100:.3f}pp"),
...
def infer_constraints(description: str, llm=None) -> Tuple[Dict, List[str]]:
    defaults = {
        "can_randomise":    True,
        "rollout_pct":      50,
    ...
    prompt = (
        f'A product team wants to test this feature: "{description}"\n\n'
        'Infer experiment setup constraints. Reply ONLY with valid JSON:\n'
        '{\n'
        '  "can_randomise": true,\n'
        '  "rollout_pct": 50,\n'
        '  "has_control_group": false,\n'
        ...
        '}\n'
        'uncertain_keys: the 1-3 keys most worth confirming with the user.'
    )
    try:
        raw  = str(llm.ask(prompt))
        ...
        logger.debug("infer_constraints LLM failed: %s", e)
...
def recommend_experiment_type(
    ...
    if can_rand and rollout <= 70:
        scores["ab_test"] += 5; reasons["ab_test"].append("Randomisation possible")
    else:
        scores["ab_test"] -= 3
    ...
    if rollout == 100 and not can_rand:
        ...
        if has_ts and pre_days >= 90:
            scores["its"] += 4; reasons["its"].append("Rich time series available")

    if has_ctrl and not can_rand:
        scores["did"] += 4; reasons["did"].append("Control group available")

    if obs:
        scores["psm"] += 4; reasons["psm"].append("Observational data")

    if thresh:
        scores["rdd"] += 5; reasons["rdd"].append("Assignment threshold detected")
    ...
    return [
        {"method": k, "reason": "; ".join(reasons[k][:2]) or EXPERIMENT_TYPE_CATALOGUE[k]["when_to_use"]}
        for k in top_3
    ]
