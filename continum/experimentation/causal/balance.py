...
def print_balance_report(
    ...
    print(f"  SMD threshold: {SMD_THRESHOLD}  (flag if SMD > {SMD_THRESHOLD} or p < 0.05 for categoricals)")
    if n_flags == 0:
...
def plot_love_plot(
    ...
    ax.axvline(SMD_THRESHOLD, color="#E74C3C", linestyle="--", lw=1.4,
...
def rerandomise_until_balanced(
    ...
    group_names = (["control"] +
                   [f"treatment_{i}" if n_groups > 2 else "treatment"
    ...
    for attempt in range(1, max_iter + 1):
        ...
        assignments_per = [len(shuffled) // n_groups + (1 if i < len(shuffled) % n_groups else 0)
