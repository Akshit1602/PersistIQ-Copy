...
import { getWorkspaceStats, MODULE_BY_ID } from '../data/moduleRegistry'
import {
  buildBriefBody,
...
export function MatchViewProvider({ children }: { children: ReactNode }) {
  ...
  const tickerMetrics = useMemo(() => getWorkspaceStats(), [])
        ...
        appendMessages([
          {
            ...
            id: nextMessageId(),
            role: 'assistant',
            content: intent.type === 'text-reply' ? intent.reply : buildAssistantReply(currentPersona, trimmed),
            timestamp: now,
          },
        ])
        return
          ...
          appendMessages([
            {
              ...
              id: nextMessageId(),
              role: 'assistant',
              content: buildAssistantReply(currentPersona, trimmed),
              timestamp: now,
            },
          ])
      }
  ...
  const openHypothesisValidator = useCallback(() => setHypothesisValidatorOpen(true), [])

  const closeHypothesisValidator = useCallback(() => setHypothesisValidatorOpen(false), [])
