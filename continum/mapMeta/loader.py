...
logger = logging.getLogger("continum.mapMeta.loader")


def _xometry_dir(data_dir: str) -> Path:
    """Xometry CSVs live under ``<data_dir>/Xometry`` (fall back to a flat
    ``<data_dir>`` for backward compatibility)."""
    p = Path(data_dir)
    sub = p / "Xometry"
    return sub if sub.exists() else p


def load_csvs(data_dir: str) -> Dict[str, pd.DataFrame]:
    from continum.mapMeta.synthetic_generator import ensure_sample_data

    xo = _xometry_dir(data_dir)
    ensure_sample_data(str(xo))
    datasets = {}
...
def build_gold_layer(db: duckdb.DuckDBPyConnection) -> None:
    ...
    db.execute(
        "CREATE OR REPLACE VIEW experiment_results AS SELECT * FROM gold_experiment_analysis"
    )
...
def register_file_datasets(db: duckdb.DuckDBPyConnection, data_dir: str = "./sample_data") -> None:
    """Load the file-backed datasets (Shell CSVs, Walmart xlsx) into DuckDB so the
    DuckDB-native AskData engine can serve every dataset from one connection."""
    root = Path(data_dir)

    def _table(name: str, df: pd.DataFrame) -> None:
        tmp = f"_{name}_src"
        db.register(tmp, df)
        db.execute(f'CREATE OR REPLACE TABLE "{name}" AS SELECT * FROM {tmp}')
        db.unregister(tmp)

    # Shell Retail — dim_station + fact_station (fact carries a %d-%m-%Y date).
    shell = root / "Shell"
    try:
        dim = pd.read_csv(shell / "Shell__dim_station__preview_.csv")
        fact = pd.read_csv(shell / "Shell__fact_station_day_product__preview_.csv")
        if "date" in fact.columns:
            fact["date"] = pd.to_datetime(
                fact["date"], format="%d-%m-%Y", errors="coerce"
            ).dt.strftime("%Y-%m-%d")
        _table("dim_station", dim)
        _table("fact_station", fact)
        logger.info("Loaded Shell dataset (dim_station, fact_station)")
    except Exception as e:  # noqa: BLE001 — optional dataset
        logger.warning("Shell dataset not loaded: %s", e)

    # Walmart retail campaign — one xlsx sheet.
    try:
        camp = pd.read_excel(root / "Walmart" / "Sample Dataset.xlsx", sheet_name="Data")
        _table("campaign_data", camp)
        logger.info("Loaded Walmart dataset (campaign_data)")
    except Exception as e:  # noqa: BLE001 — optional dataset
        logger.warning("Walmart dataset not loaded: %s", e)
...
def setup_database(data_dir: str = "./sample_data") -> duckdb.DuckDBPyConnection:
    ...
    build_gold_layer(db)
    register_file_datasets(db, data_dir)
    return db
