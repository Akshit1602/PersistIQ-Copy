...
def _publish(source: str, insight_type: str, severity: str,
             message: str, detail: str = "", **kw):
    bus = _bus()
    if bus is None:
        return
    try:
        from continum.runtime.intelligence import InsightType, InsightSeverity
        bus.emit(
            source=source,
...
def run_kpi_synthesis(
    ...
    **kwargs,
    ...
    from continum.core.intelligence.metric_planner import METRICS_KB, generate_kpi_plan
    ...
    if db is not None:
        try:
            ...
            if row and row[0] is not None:
                baselines = {"ior": float(row[0] or 0.18),
...
def run_guardrail_generation(
    ...
    **kwargs,
    ...
    if llm is not None and guardrails:
        try:
            ctx = "\n".join(f"  {g['metric']}: mean={g['baseline_mean']:.4f} ±{g['baseline_std']:.4f}" for g in guardrails)
            narrative = llm.ask(
...
def _default_guardrails(sensitivity: float = 2.0) -> List[Dict]:
    return [
        {"metric": "ior",       "baseline_mean": 0.18, "baseline_std": 0.02,
         "threshold_lo": 0.18 - sensitivity * 0.02, "threshold_hi": 0.18 + sensitivity * 0.02,
         "sensitivity_z": sensitivity, "auto_detected": False},
        {"metric": "aov",       "baseline_mean": 4000, "baseline_std": 800,
         "threshold_lo": 4000 - sensitivity * 800, "threshold_hi": 4000 + sensitivity * 800,
         "sensitivity_z": sensitivity, "auto_detected": False},
        {"metric": "daily_vol", "baseline_mean": 500, "baseline_std": 80,
         "threshold_lo": 500 - sensitivity * 80, "threshold_hi": 500 + sensitivity * 80,
         "sensitivity_z": sensitivity, "auto_detected": False},
    ]
...
def run_tracking_plan(
    ...
    **kwargs,
    ...
    from continum.core.intelligence.metric_planner import METRICS_KB
...
def run_historical_learning_retrieval(
    ...
    **kwargs,
    ...
    df["_score"] = df.apply(
        lambda r: sum(
            w for token in search_q.split()
            for w, col in [(3, "experiment_name"), (2, "key_learning"),
                           (1, "outcome"), (1, "tags")]
    ...
    if not synthesis:
        parts = [f"{r.get('key_learning', r.get('outcome', '')[:80])}" for r in matches[:3] if r.get('key_learning')]
    ...
    for r in matches[:5]:
        icon = {"ship": "✅", "no_ship": "❌", "partial_ship": "⚠️"}.get(r.get("ship_decision", ""), "⬜")
        print(f"  {icon}  [{r.get('id','?')}] {r.get('experiment_name','')}  ({r.get('recorded_at','')[:10]})")
...
_PHASE_TRANSITIONS: Dict[str, List[Dict]] = {
    "schema_discovery":       [{"module": "data_validation",   "reason": "Validate schema is complete before analysis"}],
    "data_validation":        [{"module": "dimension_setup",   "reason": "Auto-detect segments and platforms from clean data"}],
    "dimension_setup":        [{"module": "power_calculator",  "reason": "Now that baselines are set, calculate required sample size"}],
    "power_calculator":       [{"module": "opportunity_sizing","reason": "Size the revenue opportunity for the experiment"}],
    "opportunity_sizing":     [{"module": "brief_generator",   "reason": "Generate experiment brief with sizing and power inputs"}],
    "brief_generator":        [{"module": "metrics_and_tracking","reason": "Define KPIs and tracking plan"}],
    "metrics_and_tracking":   [{"module": "audience_selection","reason": "Define treatment and control segments"}],
    "audience_selection":     [{"module": "health_monitor",    "reason": "Monitor the live experiment for SRM and guardrail violations"}],
    "health_monitor":         [{"module": "sequential_testing","reason": "Check always-valid p-values for early stopping"}],
    "sequential_testing":     [{"module": "experiment_analysis","reason": "Run full post-experiment statistical analysis"}],
    "experiment_analysis":    [
    ...
    "roi_tracker":            [{"module": "uplift_modeller",   "reason": "Model per-user treatment effect for targeted rollout"}],
    "simpsons_paradox":       [{"module": "causal_analysis",   "reason": "Re-run causal analysis stratified by confounding dimension"}],
    "uplift_modeller":        [{"module": "decision_engine",   "reason": "Optimise budget-constrained targeting from uplift scores"}],
    "learnings_repository":   [{"module": "cross_experiment_learning","reason": "Synthesise patterns across all recorded learnings"}],
}
...
def run_anomaly_synthesis(
    ...
    **kwargs,
    ...
    if llm is not None and anomalies:
        ctx = "\n".join(f"  {a['metric']}: {a['type']} z={a.get('z_score','n/a')} value={a.get('value','?')} severity={a['severity']}"
                        for a in anomalies)
        try:
            synthesis = str(llm.ask(
