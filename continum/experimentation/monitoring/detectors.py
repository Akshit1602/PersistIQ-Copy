...
def detect_volume_anomaly(
    ...
    severity   = ("critical" if abs(z) > alert_z
                  else "warning" if abs(z) > 1.5
                  else "ok")
...
def detect_distribution_shift(
    ...
    severity = ("critical" if p < alert_p
                else "warning" if p < 0.01
                else "ok")

    # Which categories shifted most?
    abs_shifts = {k: round(float(today.get(k, 0) / today.sum()
                                 - baseline_counts.get(k, 0) / baseline_counts.sum()), 4)
...
def detect_freshness(
    ...
    severity = ("critical" if age_hours > sla_hours * 2
                else "warning" if age_hours > sla_hours
                else "ok")
    return {
        "status":    "analysed",
...
def detect_schema_drift(
    ...
    severity     = ("critical" if missing_cols
                    else "warning" if new_cols
                    else "ok")
    return {
...
def cross_reference_experiments(
    ...
    for exp in experiment_registry:
        ...
        try:
            ...
            end   = (pd.Timestamp(exp["end_date"])
                     if exp.get("end_date") else pd.Timestamp.now())
        except Exception:
            continue
...
def profile_dataframe(
    ...
    date_cols = [c for c in df_sample.columns if any(k in c.lower()
                  for k in ("date", "time", "at", "created", "updated", "ts"))]
