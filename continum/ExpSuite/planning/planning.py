...
from continum.ExpSuite.stats.statistics import (
    compute_duration,
    compute_opportunity,
    compute_sample_size,
)
from continum.paths import new_run_dir
from continum.userui.pdf import PDF_PALETTE, render_document_pdf

logger = logging.getLogger("continum.ExpSuite.planning.planning")
...
def run_brief_generator(llm, db=None, **kwargs) -> Dict:
    ...
    # PDF — a fresh, segregated folder per run (never a bare relative path, which
    # would land wherever the process's CWD happens to be).
    run_dir = new_run_dir("brief_generator")
    out = render_document_pdf(
        title="Experiment Brief",
        subtitle=f"Feature: {desc[:80]}",
        sections=OrderedDict(sections_raw),
        output_path=os.path.join(run_dir, f"brief_{exp_name[:30] or 'untitled'}.pdf"),
        metadata={
            "Feature": desc[:80],
            "Team": team,
            "Owner": owner,
...
def run_metrics_and_tracking(llm, db=None, **kwargs) -> Dict:
    ...
    # PDF — segregated per run (see brief_generator above for why).
    run_dir = new_run_dir("metrics_and_tracking")
    out = render_document_pdf(
        title="KPI & Data Tracking Plan",
        subtitle=f"Feature: {desc[:80]}",
        sections=OrderedDict(sections),
        output_path=os.path.join(run_dir, "metrics_tracking_plan.pdf"),
        metadata={
            "Feature": desc[:80],
            "Maturity": maturity,
            "Created": datetime.utcnow().strftime("%Y-%m-%d"),
