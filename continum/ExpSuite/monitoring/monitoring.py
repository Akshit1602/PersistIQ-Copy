...
logger = logging.getLogger("continum.ExpSuite.monitoring.monitoring")
...
def run_health_monitor(
    llm=None, db=None, experiment_registry: Optional[List[Dict]] = None, **kwargs
) -> Dict:
    print()
    print("╔" + "═" * 70 + "╗")
    ...
    for i, row in exp_df.iterrows():
        print(
            f"  [{i+1}] {row['experiment_name']}  ({row['n_rows']:,} rows  "
    ...
    for trt in treatments:
        ...
        sig = (
            f"✅ SIGNIFICANT (p={pr['p_value']:.4f})"
            if pr["is_significant"]
            else f"⏳ not yet (p={pr['p_value']:.4f})"
        )
        print(
            f"  {trt} vs {control}: Δ={pr['delta_pp']:+.4f}pp  "
    ...
    for trt in treatments:
        ...
        if n_c >= needed:
            ...
            print(
                f"  {trt}: Need {needed:,} per variant. At {daily_rate:.0f}/day "
    ...
    for g_col in guardrail_cols:
        ...
        for trt in treatments:
            ...
            print(
                f"  {g_col} ({trt} vs {control}): Δ={pct_chg:+.1f}%  "
...
def run_sequential_testing(llm=None, db=None, **kwargs) -> Dict:
    ...
    for trt in [v for v in variants if v != control]:
        ...
        status = (
            "🛑 STOP — boundary crossed (sufficient evidence)"
            if crossed
    ...
    return {"experiment": exp_name, "alpha": alpha, "threshold": threshold, "results": results}
