...
def compute_e_value(
    ...
    t = 1.0 / var_pool
    v = 1.0 / mixing_var
    e_val = float(np.sqrt(v / (v + t)) * np.exp(
        0.5 * t**2 * obs_delta**2 / (v + t)
    ))
...
class SequentialTester:
    ...
    def update(
        ...
        e_val = compute_e_value(
            n_ctrl, conv_ctrl, n_treat, conv_treat,
    ...
    def run(self, db=None, experiment_name: str = "", **kwargs) -> Dict:
        ...
        for _, row in df.iterrows():
            ...
            state = self.update(
                int(ctrl_row["n_obs"]), int(ctrl_row["n_conv"]),
                int(row["n_obs"]), int(row["n_conv"]),
            )
            results[row["variant"]] = {
