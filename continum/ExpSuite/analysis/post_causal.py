...
logger = logging.getLogger("continum.ExpSuite.analysis.post_causal")
...
def run_causal_analysis_full(
    ...
    **kwargs,
    ...
    if method in ("did", "all"):
        try:
            from continum.ExpSuite.analysis.causal.engine import run_did_v2
    ...
    if method in ("its", "all"):
        try:
            from continum.ExpSuite.analysis.causal.engine import run_its_enhanced
    ...
    if method in ("psm", "all"):
        try:
            from continum.ExpSuite.analysis.causal.engine import run_psm_full
            from continum.ExpSuite.planning.balance import plot_love_plot
    ...
    if method in ("sc", "all"):
        try:
            from continum.ExpSuite.analysis.causal.engine import run_synthetic_control_v2
    ...
    if method in ("mediation", "all"):
        try:
            from continum.ExpSuite.analysis.causal.engine import run_mediation_analysis
    ...
    if method in ("rdd", "all"):
        try:
            from continum.ExpSuite.analysis.causal.engine import run_rdd_enhanced
    ...
    if method in ("simpsons", "all"):
        try:
            from continum.ExpSuite.analysis.causal.engine import detect_simpsons_paradox
    ...
    try:
        from continum.ExpSuite.analysis.analytical_reasoning import mine_additional_insights
    ...
    if llm is not None:
        try:
            from continum.ExpSuite.analysis.analytical_reasoning import synthesise_findings
            ...
            if isinstance(m_result, dict) and not m_result.get("error"):
                ...
                bus.emit(
                    ...
                    insight_type=__import__(
                        "continum.ContextGraph.insight_bus", fromlist=["InsightType"]
                    ).InsightType.CAUSAL,
                    severity=__import__(
                        "continum.ContextGraph.insight_bus", fromlist=["InsightSeverity"]
