...
from continum.ExpSuite.stats.statistics import means_test, proportion_test

logger = logging.getLogger("continum.ExpSuite.analysis.post_analysis")
...
def run_pairwise_comparisons(
    ...
    for var_a, var_b in pairs:
        ...
        if "order_value" in df.columns:
            mr = means_test(
                ga["order_value"].values.astype(float),
                gb["order_value"].values.astype(float),
                alpha_adj,
            )
        rows.append(
            {
...
def compute_segment_slices(
    ...
    if dimensions is None:
        dimensions = [
            c for c in ["account_segment", "platform", "category", "country"] if c in df.columns
        ]
    ...
    overall_pr = proportion_test(
        len(overall_ctrl),
        int(overall_ctrl["converted_to_order"].sum()),
        len(overall_trt),
        int(overall_trt["converted_to_order"].sum()),
    )
    ...
    for dim in dimensions:
        ...
        for level in df[dim].dropna().unique():
            ...
            pr = proportion_test(
                len(c_sub),
                int(c_sub["converted_to_order"].sum()),
                len(t_sub),
                int(t_sub["converted_to_order"].sum()),
                alpha,
            )
            delta = MetricDelta(
            ...
            slices.append(
                SliceFinding(
                    experiment_id=(
                        df["experiment_name"].iloc[0] if "experiment_name" in df.columns else ""
                    ),
                    metric_name="inquiry_order_rate",
                    dimension_name=dim,
...
""")
    ...
    from continum.ExpSuite.analysis.causal.methods import (
        run_arima,
        run_bsts,
        run_did,
        run_its,
    ...
    if choice == "1":
        from continum.ExpSuite.analysis.analysis_dag import run_primary_analysis_task

        variants = sorted(df["variant"].dropna().unique())
        control = next(
            (v for v in variants if "control" in v.lower()), variants[0] if variants else "control"
        )
...
def run_pre_post_analysis(
    llm=None, df=None, db=None, intervention_date: Optional[str] = None, **kwargs
) -> Dict:
    print()
    print("╔" + "═" * 70 + "╗")
    print("║  PRE-POST ANALYSIS".ljust(71) + "║")
    print(
        "║  ⚠️  Weak causal claim — confounded by seasonality & concurrent changes".ljust(71) + "║"
    )
    print("╚" + "═" * 70 + "╝")
    print()
...
def run_simpsons_paradox_detector(llm=None, db=None, **kwargs) -> Dict:
    ...
    dims = [c for c in ["account_segment", "platform", "category", "country"] if c in df.columns]
    ...
    for trt in treatments:
        ...
        overall = proportion_test(
            len(ctrl_df),
            int(ctrl_df["converted_to_order"].sum()),
            len(trt_df),
            int(trt_df["converted_to_order"].sum()),
        )
        ...
        for dim in dims:
            ...
            for seg in df[dim].dropna().unique():
                ...
                pr_s = proportion_test(
                    len(c_s),
                    int(c_s["converted_to_order"].sum()),
                    len(t_s),
                    int(t_s["converted_to_order"].sum()),
                )
                seg_deltas.append(pr_s["delta_pp"])
                print(
                    f"    {str(seg):<22} Δ={pr_s['delta_pp']:+.3f}pp  " f"n={len(c_s)+len(t_s):,}"
                )
            ...
            if seg_deltas:
                ...
                if not same_sign:
                    ...
                    print(
                        f"  ⚠️  POSSIBLE SIMPSON'S PARADOX in {dim}: "
...
def run_roi_tracker(llm=None, db=None, **kwargs) -> Dict:
    ...
    if model is None:
        print("  ❌ Counterfactual model could not be fit.")
        return {}
    ...
    if action == "1":
        ...
        if llm is not None:
            ...
            for _, row in matches.iterrows():
                print(
                    f"  [{row['id']}] {row['experiment_name']}: {str(row.get('key_learning',''))[:80]}"
                )
    ...
    if action == "2":
        ...
        if llm is not None:
            synthesis = llm.narrate(
                {"id": new_id, "experiment": exp_name, "outcome": outcome, "learning": key_learn},
