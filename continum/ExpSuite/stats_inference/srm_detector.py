...
class SRMInput(BaseModel):
    observed_counts: List[int] = Field(
        ..., description="Observed sample counts per variant, e.g. [control, treatment]"
    )
    expected_ratios: Optional[List[float]] = Field(
        None, description="Expected allocation ratios, defaults to equal splits e.g. [0.5, 0.5]"
    )
