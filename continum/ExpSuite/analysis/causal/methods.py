...
def _make_estimate(
    ...
    from continum.ExpSuite.artifacts import CausalEstimate
...
def run_did(
    ...
    se = np.sqrt(
        var_cell(n_tp, y_tp)
        + var_cell(n_tpr, y_tpr)
        + var_cell(n_cp, y_cp)
        + var_cell(n_cpr, y_cpr)
    )
    ...
    return _make_estimate(
        ...
        method_specific={
            ...
            "n_treat_pre": n_tpr,
            "n_treat_post": n_tp,
            "y_ctrl_pre": round(y_cpr, 4),
            "y_ctrl_post": round(y_cp, 4),
            "y_treat_pre": round(y_tpr, 4),
            "y_treat_post": round(y_tp, 4),
            "twfe_used": twfe_ran,
        },
...
def run_psm(
    ...
    cov_cols = covariate_cols or [
        c for c in ["account_segment", "platform", "has_billing_profile"] if c in df.columns
    ]
    ...
    try:
        ...
        y_treat = matched_df.loc[matched_df["matched_group"] == "treatment", outcome_col].astype(
            float
        )
...
def run_rdd(
    ...
    local = data[(data["x"] >= cutoff - bandwidth) & (data["x"] <= cutoff + bandwidth)].copy()
    if len(local) < 4:
        local = data.copy()
    ...
    def _local_poly(sub, order=1):
        ...
        X = np.column_stack(
            [np.ones(len(sub))] + [sub["x_c"].values ** k for k in range(1, order + 1)]
        )
        y_ = sub["y"].values
        try:
...
def run_arima(
    ...
    fc_arr = (
        ...
        else np.array(fc.predicted_mean)
    )
    actual = post.values[: len(fc_arr)]
    causal = float(actual.mean() - fc_arr.mean()) if len(actual) > 0 else 0.0
    se = (
        float(np.std(actual - fc_arr[: len(actual)]))
        if len(actual) > 1
        else abs(causal) * 0.3 or 0.001
    )
...
def run_sarima(
    ...
    for p, q in [(1, 0), (1, 1), (0, 1)]:
        for P, Q in [(0, 0), (1, 0), (0, 1)]:
            try:
                m = SM_SARIMAX(pre.values, order=(p, 0, q), seasonal_order=(P, 0, Q, sp)).fit(
                    disp=False
                )
                if m.aic < best_aic:
                    best_aic = m.aic
    ...
    try:
        ...
        return _make_estimate(
            ...
            method_specific={
                ...
                "cumulative_effect": round(
                    float(pointwise.cumsum()[-1]) if len(pointwise) > 0 else 0.0, 6
                ),
            },
