...
def apply_cuped(
    ...
    logger.info(
        "CUPED[%s]: ρ=%.3f  θ=%.4f  var_reduction=%.1f%%  "
        "δ_adj=%.4f  p=%.4f",
        covariate_name, rho, theta, var_reduction, delta_adj, p_val,
    )
...
def apply_cupac(
    ...
    MODEL_MAP = {"ridge": Ridge(alpha=1.0), "lasso": Lasso(alpha=0.01),
                 "linear": LinearRegression()}
    clf = MODEL_MAP.get(model, Ridge(alpha=1.0))
    clf.fit(X_ctrl_s, y_ctrl)

    # R² on control as quality signal
    cv_scores = cross_val_score(clf, X_ctrl_s, y_ctrl, cv=min(5, len(y_ctrl) // 10),
                                scoring="r2")
...
def cuped_power_gain(
    ...
    return {
        ...
        "effective_mde_with_same_n": round(
            mde_abs / np.sqrt(var_reduction), 6
        ) if var_reduction > 0 else mde_abs,
    }
...
def delta_method_ratio(
    ...
    def _ratio_stats(num, den):
        ...
        if mu_x == 0:
            ...
            var_ratio = (var_y / mu_x**2
                         - 2 * mu_y * cov_xy / mu_x**3
                         + mu_y**2 * var_x / mu_x**4) / n
...
def bootstrap_ci(
    ...
    if stat_fn is None:
        stat_fn = lambda c, t: float(t.mean() - c.mean())
    ...
    jack = np.array([stat_fn(np.delete(ctrl, i) if i < len(ctrl)
                             else ctrl, treat)
    ...
    alpha_lo = float(stats.norm.cdf(
        z0 + (z0 + stats.norm.ppf(alpha / 2)) / (1 - accel * (z0 + stats.norm.ppf(alpha / 2)))))
    alpha_hi = float(stats.norm.cdf(
        z0 + (z0 + stats.norm.ppf(1 - alpha / 2)) / (1 - accel * (z0 + stats.norm.ppf(1 - alpha / 2)))))
    ...
    p_val = float(min(1.0, 2 * min(
        (boot_stats >= observed).mean(),
        (boot_stats <= observed).mean()) + 1e-9))
...
__all__ = [
    "CupedResult", "CupacResult",
    "apply_cuped", "apply_cupac",
    "cuped_power_gain", "delta_method_ratio",
    "bootstrap_ci",
]
