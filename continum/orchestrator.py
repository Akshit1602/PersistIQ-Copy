"""
MatchView tool-calling layer for the Continum Copilot.

The Copilot is the UI layer over the MatchView / Continum experimentation
platform. When a user's request maps to a MatchView application module, this
layer:
...
def execute_tool(
    ...
    ``{response, sql, table, columns, visualizations, error}``.
    """
...
def _summarise_result(result: Any, tool: MatchViewTool) -> str:
    ...
    try:
        # Experiment-readout dataclass (verdict + primary delta).
...
# ===== relocated from former runtime.shell.executor (used by userui) =====
