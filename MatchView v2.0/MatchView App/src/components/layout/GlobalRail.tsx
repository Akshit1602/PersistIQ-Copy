...
export function GlobalRail() {
  const { goHome, selectedProjectId, openKnowledgeArchive } = useMatchView()
  const [hovered, setHovered] = useState(false)
  const onHome = selectedProjectId === null

  const handleClick = (id: 'home' | 'archive' | 'settings') => {
    if (id === 'home') goHome()
    else if (id === 'archive') openKnowledgeArchive()
    // 'settings' intentionally has no destination yet
  }
