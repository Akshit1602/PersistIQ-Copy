...
def _date_range(last_date: Any, horizon: int, freq: str = "D") -> List[str]:
    ...
    return [
        str((last + pd.tseries.frequencies.to_offset(freq) * (i + 1)).date())
...
def run_forecast_ensemble(
    ...
    fc_mean = [sum(results[m]["forecast"][i] * weights[m] for m in results) for i in range(horizon)]
    ci_lo = [sum(results[m]["ci_lo"][i] * weights[m] for m in results) for i in range(horizon)]
    ci_hi = [sum(results[m]["ci_hi"][i] * weights[m] for m in results) for i in range(horizon)]
...
def run_causal_impact(
    ...
    fc_fn = {"bsts": run_bsts_forecast, "arima": run_arima_forecast, "ets": run_ets_forecast}.get(
        method, run_ets_forecast
    )
