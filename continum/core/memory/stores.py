...
class ExecutionMemory:
    ...
    def record_job_start(self, job_id: str, dag_name: str, params: Optional[Dict] = None) -> None:
        self._job_runs[job_id] = {
            "dag_name": dag_name, "status": "running",
            "started_at": datetime.utcnow().isoformat(), "params": params or {},
        }
    ...
    def record_job_failed(self, job_id: str, error: str) -> None:
        if job_id in self._job_runs:
            self._job_runs[job_id].update({
                "status": "failed", "failed_at": datetime.utcnow().isoformat(), "error": error,
            })
    ...
    def get_last_run(self, dag_name: str) -> Optional[str]:
        runs = [r for r in self._job_runs.values()
    ...
    def set_cache(self, key: str, value: Any, ttl_seconds: int = 3600) -> None:
        self._cache[key] = {"value": value, "expires_at": datetime.utcnow().timestamp() + ttl_seconds}
    ...
    def record_retry(self, task_id: str, attempt: int, error: str) -> None:
        self._retries.setdefault(task_id, []).append({
            "attempt": attempt, "error": error, "at": datetime.utcnow().isoformat(),
        })
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"store": self._store, "job_runs": self._job_runs,
                "retries": self._retries, "baselines": self._baselines}
...
class OrganisationalMemory:
    def __init__(self, knowledge_graph=None):
        if knowledge_graph is None:
            from continum.core.intelligence.knowledge_graph import KnowledgeGraph
    ...
    def set_client_config(self, client_name: str, schema_version: int,
                           column_map: Dict, segment_map: Dict,
                           platform_map: Dict, approved_by: str) -> None:
        config = {
            "client_name": client_name, "schema_version": schema_version,
            "column_map": column_map, "segment_map": segment_map,
            "platform_map": platform_map, "approved_by": approved_by,
    ...
    def get_analysis_dimensions(self) -> List[str]:
        return self._dimension_config.get("analysis_dimensions",
                                          ["account_segment", "platform", "category", "country"])

    def get_segments(self) -> List[str]:
        return self._dimension_config.get("segments", ["Individuals", "Core", "Growth", "Enterprise"])

    def get_platforms(self) -> List[str]:
        return self._dimension_config.get("platforms", ["web", "mobile"])

    def store_learning(self, statement: str, domain: str, experiment_ids: List[str],
                        confidence: str = "tentative", evidence_type: str = "correlational",
                        applicable_metrics: Optional[List[str]] = None,
                        applicable_segments: Optional[List[str]] = None,
                        owner: str = "") -> str:
        from uuid import uuid4
        learning_id = str(uuid4())
        self._kg.upsert_node("Learning", learning_id, {
            "statement": statement, "domain": domain, "confidence": confidence,
            "evidence_type": evidence_type, "applicable_metrics": applicable_metrics or [],
            "applicable_segments": applicable_segments or [], "experiment_ids": experiment_ids,
    ...
    def get_relevant_learnings(self, metric: Optional[str] = None,
                                domain: Optional[str] = None,
                                min_confidence: str = "tentative") -> List[Dict]:
        return self._kg.get_learnings_for_context(metric=metric, domain=domain,
        ...
        try:
            ...
            if org_cfg.get("client_config"):
                ...
                state.org.set_client_config(
                    client_name=cc.get("client_name", ""), schema_version=cc.get("schema_version", 1),
                    column_map=cc.get("column_map", {}), segment_map=cc.get("segment_map", {}),
                    platform_map=cc.get("platform_map", {}), approved_by=cc.get("approved_by", ""),
                )
