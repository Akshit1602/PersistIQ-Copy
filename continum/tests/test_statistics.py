...
class TestCUPED:
    def test_variance_reduction_positive(self, cuped_arrays):
        ...
        r = apply_cuped(
            ...
            cuped_arrays["post_treat"],
            cuped_arrays["pre_ctrl"],
            cuped_arrays["pre_treat"],
        )
    ...
    def test_adjusted_delta_closer_to_truth(self, cuped_arrays):
        ...
        true_delta = cuped_arrays["true_delta"]  # noqa: F841
    ...
    def test_power_gain_formula(self):
        ...
        expected_saving = (1 - (1 - 0.7**2)) * 100  # noqa: F841
    ...
    def test_bootstrap_ci_contains_true_delta(self, positive_experiment, rng):
        ...
        true_delta = positive_experiment["true_rate_treat"] - positive_experiment["true_rate_ctrl"]
        r = bootstrap_ci(ctrl, treat, n_boot=1000, alpha=0.05, seed=7)
        assert r["ci_lo_bca"] <= true_delta <= r["ci_hi_bca"], (
            f"True delta {true_delta} not in BCa CI " f"[{r['ci_lo_bca']}, {r['ci_hi_bca']}]"
        )
...
class TestSequential:
    ...
    def test_sequential_tester_state_accumulates(self):
        ...
        for batch in range(5):
            state = tester.update(  # noqa: F841
                100 * (batch + 1), 18 * (batch + 1), 100 * (batch + 1), 20 * (batch + 1)
            )
...
class TestGuardrails:
    ...
    def test_relative_harm_detected(self, rng):
        ...
        specs = [
            GuardrailSpec(
                "checkout_no_harm", "checkout_rate", "relative_min", -0.10, "breach", alpha=0.05
            )
        ]
        result = run_guardrail_checks("treat", ctrl, treat, specs)
...
class TestDetectors:
    ...
    def test_profile_dataframe_numeric_stats(self, revenue_arrays):
        ...
        df = pd.DataFrame(
            {
                "order_value": revenue_arrays["ctrl"],
                "variant": ["control"] * len(revenue_arrays["ctrl"]),
            }
        )
