...
def module_label(key: str) -> str:
    """The canonical display name for a module key — reads the registry's
    ``display_name`` (the single source of truth; see
    continum.ExpSuite.registry) so guided-flow prose always matches the
    dashboard card and the MatchView tool confirmation for the same module."""
    try:
    ...
    except Exception:  # noqa: BLE001 — never let a naming lookup break the flow
        pass
...
def format_question(
    ...
    idx = int(state.get("idx", 0))
    f = fields[idx]
    # Always use the canonical name here (not config["title"], which is a
    # decorative form-header string that can diverge from the dashboard card).
    title = module_label(state["module_key"])
