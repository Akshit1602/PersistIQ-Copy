...
logger = logging.getLogger("continum.ExpSuite.analysis.compare")
...
def _render_comparison(name_a: str, m_a: Dict, name_b: str, m_b: Dict) -> None:
    ...
    def _sig(m):
        return "✅ Yes" if m.get("is_sig") else "❌ No"

    def _srm(m):
        return "⚠️  Detected" if m.get("srm") else "✅ Clean"
    ...
    rows = [
        ...
        (
            "N (ctrl/treat)",
            f"{m_a.get('n_control',0):,}/{m_a.get('n_treatment',0):,}",
        ...
        (
            "Guardrail Viols",
            str(len(m_a.get("guardrails", []))),
            str(len(m_b.get("guardrails", []))),
        ),
    ]
