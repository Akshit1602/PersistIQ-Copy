...
# ─────────────────────────────────────────────────────────────────────────────
# SIMPSON'S PARADOX NARRATIVE
# ─────────────────────────────────────────────────────────────────────────────

def generate_paradox_narrative(
    overall_delta: float,
    paradoxes: List[Dict],
    llm=None,
) -> str:
    if not paradoxes:
        return "No Simpson's Paradox detected — overall and segment-level effects are consistent."

    if llm is not None:
        ctx = (
            f"Overall effect: {overall_delta:+.4f}pp\n"
            f"Number of paradoxes detected: {len(paradoxes)}\n"
        )
        for p in paradoxes[:2]:
            ctx += (
                f"Dimension: {p['dimension']}, "
                f"segment effects: {[round(d, 4) for d in p['segment_deltas'][:4]]}\n"
            )
        prompt = (
            "Explain Simpson's Paradox to a product manager in 3-4 sentences. "
            "Be specific about what the data shows and why the overall metric "
            "is misleading for this audience.\n\n"
            f"DATA:\n{ctx}"
        )
        try:
            return str(llm.ask(prompt))
        except Exception as e:
            logger.warning("Paradox narrative LLM failed: %s", e)

    p0   = paradoxes[0]
    dim  = p0["dimension"]
    segs = p0["segment_deltas"]
    pos  = sum(1 for d in segs if d > 0)
    neg  = sum(1 for d in segs if d < 0)

    return (
        f"Simpson's Paradox was detected in the '{dim}' dimension. "
        f"The overall metric moved {overall_delta:+.4f}pp, but "
        f"{pos} segment(s) moved positively and {neg} negatively within that dimension. "
        f"This means the aggregate effect is driven by changes in the mix of users "
        f"across segments, not by a uniform improvement. "
        f"Reporting the overall number without segment context is misleading — "
        f"investigate which segments are driving the effect before making a ship decision."
    )
