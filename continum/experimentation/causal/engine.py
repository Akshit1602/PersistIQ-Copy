...
def run_did_v2(
    ...
    def ior(d): return float(d[outcome_col].mean()) if len(d) > 0 else np.nan
    ior_tp, ior_tpo = ior(cells["tp"]), ior(cells["tpo"])
    ior_cp, ior_cpo = ior(cells["cp"]), ior(cells["cpo"])
    did_est = (ior_tpo - ior_tp) - (ior_cpo - ior_cp)

    se_delta = np.sqrt(
        sum(_proportion_se(ior(cells[k]), len(cells[k])) ** 2
            for k in ("tp", "tpo", "cp", "cpo"))
    )
    z_crit = norm.ppf(1 - alpha / 2)
    ...
    try:
        X_pt = np.column_stack([
            np.ones(len(pre_data)), pre_data["t"].values,
    ...
    for wk in range(-12, 17):
        ...
        if len(tw) >= 15 and len(cw) >= 15:
            gap   = ior(tw) - ior(cw)
            se_g  = np.sqrt(_proportion_se(ior(tw), len(tw))**2 +
                            _proportion_se(ior(cw), len(cw))**2)
            event_study.append({
                "week":       wk,
    ...
    if run_twfe:
        try:
            ...
            panel = (data.groupby([unit_col, "week", "treated"])[outcome_col]
                     .mean().reset_index())
            ...
            if np.sum(X_tw**2) > 1e-10:
                ...
                se_tw   = float(np.sqrt(np.sum(resid2**2) / max(len(y_tw)-2,1) / np.dot(X_tw,X_tw)))
    ...
    return {
        ...
        "ci_delta_pp":               [round((did_est - z_crit*se_delta)*100,4),
        ...
        "parallel_trends_note": (
            f"✅ Parallel trends holds (p={pt_p:.3f})" if pt_p > 0.10
            else f"⚠️ Parallel trends VIOLATED (p={pt_p:.3f}) — DiD estimate may be biased."
        ),
        "event_study":  event_study,
        **twfe_result,
        "n_treat_pre":  len(cells["tp"]),  "n_treat_post": len(cells["tpo"]),
        "n_ctrl_pre":   len(cells["cp"]),  "n_ctrl_post":  len(cells["cpo"]),
    }
...
def run_psm_full(
    ...
    p_val_res = _z_test(float(c_out.mean()), len(c_out),
                        float(t_out.mean()), len(t_out), alpha)
    ...
    for col_idx, col_name in enumerate(col_names):
        ...
        smd_before.append({"covariate": col_name,
                            "smd": round(abs(raw_t.mean() - raw_c.mean()) / pool_sd, 4)})
        smd_after.append({"covariate": col_name,
                           "smd": round(abs(mat_t.mean() - mat_c.mean()) / pool_sd, 4)})
    ...
    return {
        ...
        "balance_note": (
            "✅ Good balance (max SMD < 0.10)" if max_smd < 0.10
...
def run_synthetic_control_v2(
    ...
    data["week"] = data[date_col].dt.to_period("W").apply(lambda x: x.start_time)
    panel = (data[data[unit_col].isin([treatment_unit] + donor_units)]
             .groupby(["week", unit_col])[outcome_col].mean().reset_index())
    ...
    bounds      = [(0, 1)] * n_d
    res         = _minimize(_loss, w0, method="SLSQP",
    ...
    for d in donor_units[:n_placebo_permutations]:
        ...
        res_p   = _minimize(lambda w: float(np.mean((Y_d_pre - D_pre @ w)**2)),
    ...
    p_value_perm = (
        float(np.mean([r >= rmspe_ratio for r in placebo_ratios]))
        if placebo_ratios else None
    )
    ...
    return {
        ...
        "fit_note": (
            ...
            "fit quality too poor to extrapolate" if fit_warn
...
def run_mediation_analysis(
    ...
    for i in range(n_bootstrap):
        ...
        b_t   = t_flag[idx]; b_m = M[idx]; b_y = Y[idx]
...
def run_rdd_enhanced(
    ...
    if len(left) < 10 or len(right) < 10:
        return {"error": f"Insufficient data near cutoff (bw={bandwidth:.3f}): "
    ...
    se_disc = np.sqrt((se_right * np.sqrt(len(right)))**2 / len(right) +
    ...
    manipulation_flag = (density_ratio is not None and
                         (density_ratio > 2.0 or density_ratio < 0.5))
    ...
    return {
        ...
        "manipulation_note": (
            ...
            if manipulation_flag else "✅ No evidence of cutoff manipulation"
        ),
...
def detect_simpsons_paradox(
    ...
    dims = dimensions or [c for c in ["account_segment", "platform", "country", "category"]
                          if c in data.columns]
    ...
    overall_sig   = len(data) > 200  # rough proxy
    ...
    return {
        ...
        "verdict": (
            ...
            if paradoxes else
            "✅ No Simpson's paradox detected. Segment effects are consistent with aggregate."
        ),
    }
...
def detect_distribution_shift(
    ...
    for col in cols:
        try:
            ...
            if is_numeric:
                stat, p = scipy_stats.ks_2samp(pre_vals.values.astype(float),
                                                post_vals.values.astype(float))
                test = "ks"
            else:
    ...
    return {
        ...
        "overall_severity": (
            "critical" if any(r["severity"] == "critical" for r in shifted)
