...
try:
    ...
    from continum.ExpSuite.stats.sequential import (
        SequentialTester,  # noqa: F401
        compute_e_value,
    )  # noqa: F401
    from continum.ExpSuite.stats.srm_detector import (
        detect_dimensional_srm,  # noqa: F401
        detect_srm,
    )  # noqa: F401
...
def run_primary_analysis_task(
    ...
    from continum.ExpSuite.artifacts import (
        ExperimentResult,
        MetricDelta,
        ShipRecommendation,
        SignificanceVerdict,
    )
    from continum.ExpSuite.stats.statistics import check_srm, means_test, proportion_test
    ...
    n_t = len(treat_df)
    c_t = int(treat_df["converted_to_order"].sum())
        ...
        if "error" not in mr:
            secondary_deltas.append(
                MetricDelta(
                    ...
                    control_variant=control_variant,
                    treatment_variant=treatment,
                    n_control=mr["n_control"],
                    n_treatment=mr["n_treatment"],
                    mean_control=mr["mean_control"],
                    mean_treatment=mr["mean_treatment"],
                    delta_abs=mr["delta_abs"],
                    delta_rel=mr["delta_rel"],
                    ci_lo=mr["ci_lo"],
                    ci_hi=mr["ci_hi"],
                    p_value=mr["p_value"],
                    effect_size=mr["effect_size_d"],
    ...
    if srm["srm_detected"]:
        ...
        ship = (
            ...
            if pr["direction"] == "positive"
            else ShipRecommendation.DO_NOT_SHIP
        )
    ...
    if _DEEP_STATS:
        try:
            ...
            e_value_result = {
                "e_value": e_val,
                "p_msprt": min(1.0, 1.0 / max(e_val, 1e-10)),
                "boundary_crossed": e_val >= 1.0 / alpha,
            }
            logger.info(
                "Sequential[%s]: e_value=%.4f boundary=%s",
                experiment_id,
                e_val,
                e_value_result["boundary_crossed"],
            )
        except Exception as _e:
...
def run_segment_slicing_task(
    ...
    try:
        from continum.ExpSuite.analysis.segment import run_segment_analysis
...
def run_causal_suite_task(
    ...
    from continum.ExpSuite.analysis.causal.methods import run_did, run_psm
...
def check_guardrails_task(
    ...
    from continum.ExpSuite.artifacts import GuardrailViolation
...
def save_result_json_task(
    ...
    out_path = output_path or os.path.join(
        new_run_dir("experiment_analysis"), f"result_{exp_id}_{ts}.json"
    )
...
def _build_template_narrative(result: Any, causal_estimates: List, violations: List) -> str:
    ...
    if result.srm_detected:
        lines.append(
            f"WARNING: SRM detected (p={result.srm_p_value:.4f}) — results may be unreliable."
        )
    if causal_estimates:
        est = causal_estimates[0]
        lines.append(
            f"Causal estimate ({est.method}): {est.estimate:+.4f} "
    ...
    if result.slice_findings:
        ...
        if sig_slices:
            s = sig_slices[0]
            lines.append(
                f"Notable segment: {s.dimension_name}={s.dimension_value} "
...
def run_experiment_analysis_pipeline(
    ...
    try:
        ...
        pipeline_log["tasks"]["causal_suite"] = {
            "status": "ok",
            "n_estimates": len(causal_estimates),
        }
        ...
        pipeline_log.update({"status": "completed", "completed_at": datetime.utcnow().isoformat()})
