...
from continum.mapMeta import fetch_statsig_experiment_health, StatSigFetchInput
from continum.askdata import execute_sql_query, SQLExecutionInput
...
@mcp_server.tool()
def check_sample_ratio_mismatch(control_count: int, treatment_count: int, expected_ratio: float = 0.5) -> str:
    """Check for Sample Ratio Mismatch (SRM) across experiment variants using Chi-Square goodness-of-fit."""
    res = detect_srm(SRMInput(observed_counts=[control_count, treatment_count], expected_ratios=[expected_ratio, 1.0 - expected_ratio]))
    return res.summary


@mcp_server.tool()
def calculate_sample_size_and_power(baseline_rate: float, mde_relative: float, alpha: float = 0.05, power: float = 0.80) -> str:
    """Calculate required sample size per variant and total test duration given baseline rate and relative MDE."""
    res = calculate_power(PowerCalcInput(baseline_rate=baseline_rate, mde_relative=mde_relative, alpha=alpha, power=power))
    return res.summary


@mcp_server.tool()
def calculate_opportunity_sizing(annual_traffic: int, baseline_conversion_rate: float, average_order_value: float, assumed_relative_lift: float = 0.02) -> str:
    """Estimate annual and quarterly incremental conversions and revenue impact for a target lift hypothesis."""
    res = calculate_opportunity_size(OpportunitySizingInput(
...
@mcp_server.tool()
def run_hypothesis_test(control_mean: float, control_std: float, control_count: int, treatment_mean: float, treatment_std: float, treatment_count: int) -> str:
    """Compute Welch's t-test / Z-test, confidence intervals, and relative lift between Control and Treatment."""
    res = calculate_hypothesis_test(StatTestInput(
