...
class TestTypeIError:
    ...
    def test_proportion_test_type1_nominal_n(self):
        ...
        assert abs(fpr - ALPHA) < TOLERANCE_PP, (
    ...
    def test_proportion_test_type1_small_n(self):
        ...
        rejections = sum(
            ...
            if proportion_test(100, rng.binomial(100, 0.18),
                               100, rng.binomial(100, 0.18))["is_significant"]
        )
        fpr = rejections / N_SIMS_SLOW
    ...
    def test_means_test_type1(self):
        ...
        rejections = sum(
            ...
            if means_test(rng.exponential(4000, 500),
                          rng.exponential(4000, 500))["is_significant"]
        )
        fpr = rejections / N_SIMS_FAST
        assert abs(fpr - ALPHA) < TOLERANCE_PP + 0.02, (
...
class TestPower:
    ...
    def test_power_at_designed_n(self):
        ...
        detections = sum(
            1 for _ in range(N_SIMS_FAST)
            if proportion_test(n, rng.binomial(n, baseline),
                               n, rng.binomial(n, baseline + mde_abs))["is_significant"]
        )
        empirical_power = detections / N_SIMS_FAST
        assert empirical_power >= POWER_TARGET - 0.07, (
    ...
    def test_larger_effect_has_higher_power(self):
        ...
        def _emp_power(mde):
            return sum(
                1 for _ in range(N_SIMS_FAST)
                if proportion_test(n, rng.binomial(n, 0.18),
                                   n, rng.binomial(n, 0.18 + mde))["is_significant"]
            ) / N_SIMS_FAST
    ...
    def test_more_n_monotonically_increases_power(self):
        ...
        for n in [100, 300, 1000, 3000]:
            det = sum(
                1 for _ in range(N_SIMS_FAST)
                if proportion_test(n, rng.binomial(n, 0.18),
                                   n, rng.binomial(n, 0.20))["is_significant"]
            )
...
class TestCupedCalibration:
    ...
    def test_variance_reduction_matches_rho_squared(self):
        ...
        assert abs(avg_reduction - expected) < 15, (
    ...
    def test_no_variance_reduction_without_covariate(self):
        ...
        assert avg < 10, (
            f"Random covariate should not reduce variance: mean={avg:.1f}%")
    ...
    def test_msprt_valid_under_repeated_peeking(self):
        ...
        for _ in range(N_SIMS_MEDIUM):
            ...
            for look in range(n_looks):
                ...
                state = tester.update(
                    (look + 1) * n_per_look, int(data_c[sl].sum()),
                    (look + 1) * n_per_look, int(data_t[sl].sum()),
                )
        ...
        assert fpr <= ALPHA + TOLERANCE_PP, (
            f"Sequential peeking type-I error = {fpr:.3f}, "
    ...
    def test_obrien_fleming_alpha_spending_monotone(self):
        ...
        for i in range(len(spends) - 1):
            assert spends[i] <= spends[i+1], (
                f"OBF alpha not monotone: {spends}")
...
class TestSRMCalibration:
    ...
    def test_srm_severity_correlates_with_imbalance(self):
        ...
        assert severity_rank[severe_imbalance.severity.value] >= \
