...
class CausalAnalysisInput(BaseModel):
    analysis_type: str = Field(
        "diff_in_diff", description="Type of observational analysis: 'diff_in_diff' or 'forecast'"
    )
...
def run_causal_engine(input_data: CausalAnalysisInput) -> CausalAnalysisResult:
    ...
    if input_data.analysis_type == "diff_in_diff" and input_data.did_input:
        ...
        return CausalAnalysisResult(
            analysis_type="diff_in_diff", did_result=did_res, summary=did_res.summary
        )
        ...
        return CausalAnalysisResult(
            analysis_type="forecast", forecast_result=forecast_res, summary=forecast_res.summary
        )
    else:
        return CausalAnalysisResult(
